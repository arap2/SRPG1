...
CDS_counter = 0
covid_dictionary = {}
filename = "covid01_03-31_03.gb"
part = 1

CDS_sequences = []
for rec in SeqIO.parse(filename, "genbank"):
	if(record_counter%500) == 0 and record_counter != 0:
		print(str(record_counter), "A NEW FILE HAS BEEN CREATED") # splitting the output into
		output_file = f"new_{filename[:-3]}_out_part{part}.json"  # separate files to overcome
		with open(output_file, "w") as data_file:				  # the memory error on loading
			json.dump(covid_dictionary, data_file)				  # json files on the lamp server
		part +=1
		covid_dictionary = {}
	record_counter +=1
	CDS_counter = 0
